...
#define dirPin 4    // Tyler = 2
#define stepPin 5   // Tyler = 3
...
void calculation(){
  ...
  flowRate = doseOrderedHr*volume/ammount; //ml/hr delivery rate
...
void provideDose(){
  stepper.moveTo(100);//numSteps);      //Set the position to go to (non-blocking)
  stepper.setSpeed(1);//calcSpeed);     //Set speed of motor (non-blocking)
  // Serial.println(numSteps);
  // Serial.println(calcSpeed);
  while(stepper.isRunning()){           //blocking while loop
    stepper.runSpeedToPosition();       //non-blocking <-- should be in greater loop rather than the provide Dose
  }
...
void setup() {
  ...
  delay(5000);
  // attachInterrupt(digitalPinToInterrupt(outputA), encoderIterrupt, FALLING);
}
